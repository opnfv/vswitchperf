...
class IVSwitchOvs(IVSwitch, tasks.Process):
    ...
    def __init__(self):
        ...
        self._ovsdb_pidfile_path = os.path.join(settings.getValue('TOOLS')['ovs_var'],
                                                "ovsdb-server.pid")
        self._vswitchd_pidfile_path = os.path.join(settings.getValue('TOOLS')['ovs_var'],
    ...
    def _reset_ovsdb(self):
        ...
        tasks.run_task(['sudo', 'rm', '-rf', settings.getValue('TOOLS')['ovs_var']], self._logger)
        tasks.run_task(['sudo', 'mkdir', '-p', settings.getValue('TOOLS')['ovs_var']], self._logger)
        tasks.run_task(['sudo', 'rm', '-rf', settings.getValue('TOOLS')['ovs_etc']], self._logger)
        tasks.run_task(['sudo', 'mkdir', '-p', settings.getValue('TOOLS')['ovs_etc']], self._logger)

        tasks.run_task(['sudo', 'rm', '-f',
                        os.path.join(settings.getValue('TOOLS')['ovs_etc'], 'conf.db')],
                       self._logger)
    ...
    def _start_ovsdb(self):
        ...
        tasks.run_background_task(
            ['sudo', ovsdb_server_bin,
             '--remote=punix:%s' % os.path.join(settings.getValue('TOOLS')['ovs_var'], 'db.sock'),
    ...
    def get_db_sock_path():
        ...
        """
        return os.path.join(settings.getValue('TOOLS')['ovs_var'], 'db.sock')
