...
- **Packet delay variation**: measured from the perspective of the
  ...
  measurements of PDV consider the delay variation of a single user flow,
  as this will be relevant to the size of end-system buffers to compensate
  for delay variation. The measurement systems ability to store the 
  ...
  distribution accurately. Systems without storage must apply algorithms 
  to delay measurements on the fly. For example, temporary views of the
  mimimum delay and the set of (100) packets with the longest delays, and 
  update these sets with new values periodically during measurement. 
