...
**NOTE** By default the GUEST_THREAD_BINDING will be none, which means the same
as GUEST_CORE_BINDING, i.e. the vcpu threads are sharing the physical CPUs with
the house keeping threads Increase the performance using thread binding can be
done by enabling affinity in the ''04_vnf.conf'' file.
