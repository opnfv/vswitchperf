...
|Type        |            |                                                    |
+------------+------------+----------------------------------------------------+
|Deployment  | true|false | Specifies if genearted traffic will be full-duplex |
...
|            |            | during continuous stream tests.                    |
+------------+------------+----------------------------------------------------+
| Load       | Dictionary | Configures background load for testcase execution  |
+------------+------------+----------------------------------------------------+
| load       | 0-100      | percentage of cores which should be utilized by    |
|            |            | load generator.                                    |
+------------+------------+----------------------------------------------------+
...
|            |            |"ccccmmi" => ratio among workers types will be 3:2:1|
...
1. Creating a testcase with Different configuration using existing supported values. This case is straight forward, and involves modifying the configurable parameters. You will have new testcase, with the given configuration. The configuration file can be found in: ``$HOME/conf/01_testcases.conf``.
...
==============================
As a case-study, consider you are new traffic generator vendor by name ``Super``, and consider you want to support RFC2819 traffic type. The complete process of adding a new testcase in this scenario can be explained in following steps:
...
- Ensure that you name the classes and files appropriately. In this case, the class name would be ``class TrafficControllerRFC2819(ITrafficController, IResults)`` and the file name would be ``traffic_controller_rfc2819.py``. Place the file in ``$HOME/core/``. 
...
- Add appropriate import: in this case, it would be ``from core.traffic_controller_rfc2819 import TrafficControllerRFC2819``
- In ``create_traffic`` function, make necessary changes. Check for the type, if it is rfc2819, and return ``TrafficControllerRFC2819(trafficgen_class)`` object

4. Configurations Required:
---------------------------
Finally, the configuration specific changes that are required are:
...
| "Name": "phy2phy_2819"
| "Traffic Type": "rfc2819"
| "Deployment": "p2p",
| "biDirectional": "True",
| "Description": " RFC 2819 Test by Super"
