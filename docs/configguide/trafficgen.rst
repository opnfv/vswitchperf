...
For VSPerf use, MoonGen should be cloned from here (as opposed to the afore mentioned GitHub):
...
VSPerf uses a particular example script under the examples directory within the 
MoonGen project:
...
Note one will need to set up ssh login to not use passwords between the server running MoonGen and the device under test (running the VSPERF test infrastructure)
