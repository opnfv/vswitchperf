...
(or you are existing vendor) and you want to support RFC2889 traffic type.
The complete process of adding a new testcase in this scenario can be
explained in the following steps:

1. Implementation/Enhancement of Traffic Generator:
---------------------------------------------------
The process of integrating new traffic generator is *well documented* in
``trafficgen_integration_guide.rst``, which explains how to add new traffic
generator in detail. For the sake of completion, the below points are added
to summarize this step considering both the cases - existing traffic generator
or new traffic generator.

a. Configuration parameters: The important configuration parameters to consider
are  ``TRAFFICGEN_DIR: $HOME/tools/pkt_gen/`` and ``TRAFFICGEN: Super``

b. Create a new trafficgen folder in ``$HOME/tools/pkt_gen`` and assign it to
``TRAFFICGEN`` parameter. Consider you create a folder named ``Super`` for
your Super traffic-generator.

**Note**: In case  you want to consider existing folders ,
for ex Ixia or Spirent's testcenter folder, (if you represent these
organizations!) this step is not required.

c. Create a class that inherits from ``trafficgen.ITrafficGenerator`` class.
Ex: Create a file ``SuperTraffic.py`` and add
``class SuperTraffic(trafficgen.ITrafficGenerator)``.

**Note**: Again, if you are modifying the file in existing folder
(Ixia/Spirent), just update it to support new traffic generation functions.

d. Implement Traffic generation functions - add function to the class that will
generate actual traffic. For example, in Class ``SuperTraffic``, implement the
functions ``send/connect/disconnect``, appropriately.

**Note**: These functions will be called from the traffic controller.
