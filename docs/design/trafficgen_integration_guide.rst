...
recommended to create a new directory for new traffic generator
implementations.
...
Function **connect** is called from the traffic generator controller from its
...
Complementary to **connect** is function **disconnect**. It should perform clean up
of any connection specific actions called from **connect**.
...
these are identified in vsperf through the traffic type, which include:
    ...
    * RFC2544 throughput - Send packets at a variable rate, using
    ...
    * RFC2544 back2back - Send packets at a fixed rate, using traffic
        configuration, for specified time interval. Methods with its
        implementation have suffix **_rfc2544_back2back**.

    * continuous flow - Send packets at given framerate, using traffic
        configuration, for specified time interval. Methods with its
