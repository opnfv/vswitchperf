...
def _get_user_traffic_stat(stat_type):
    ...
    while True:
        result = raw_input('What was the result for \'%s\'? ' % stat_type)
        ...
        while True:
            choice = raw_input('Is \'%d\' correct? ' % result).lower()
...
class Dummy(trafficgen.TrafficGenerator):
    """
    ...
    def send_burst_traffic(self, traffic=None, numpkts=100, time=20):
        ...
        results = [
            numpkts,  # frames tx
            results[0],  # frames rx
            traffic_['l2']['framesize'] * numpkts,  # bytes rx
            traffic_['l2']['framesize'] * results[0],  # bytes tx
            results[1],  # payload errors
            results[2]  # sequence errors
        ]
    ...
    def send_cont_traffic(self, traffic=None, time=20, framerate=0):
        ...
        # and guessing remainder using available info
        results = [
            float(results[0]) / time,  # tx throughput (fps)
            float(results[1]) / time,  # rx throughput (fps)
            (float(results[0]) * framesize) / time,  # tx throughput (mbps)
            (float(results[1]) * framesize) / time,  # rx throughput (mbps)
            0.0,  # tx throughput (% linerate)
            0.0,  # rx throughput (% linerate)
            float(results[2]),  # min latency
            float(results[3]),  # max latency
            float(results[4])  # avg latency
        ]

        return trafficgen.ContResult(*results)
    ...
    def send_rfc2544_throughput(self, traffic=None, trials=3, duration=20,
        ...
        # and guessing remainder using available info
        results = [
            float(results[0]) / duration,  # tx throughput (fps)
            float(results[1]) / duration,  # rx throughput (fps)
            (float(results[0]) * framesize) / duration,  # tx throughput (mbps)
            (float(results[1]) * framesize) / duration,  # rx throughput (mbps)
            0.0,  # tx throughput (% linerate)
            0.0,  # rx throughput (% linerate)
            float(results[2]),  # min latency
            float(results[3]),  # max latency
        ...
        return trafficgen.ThroughputResult(*results)
