...
def get_results_to_print(dict_of_arrays):
    ...
    for key in dict_of_arrays:
        if 'processes.ovs' in key:
            ...
            results[reskey] = mean
            if 'processes.ovs-vswitchd' in key:
                presults['ovs-vswitchd_collectd'] = results
            elif 'processes.ovsdb-server' in key:
                presults['ovsdb-server_collectd'] = results
    return presults
...
class Receiver(multiprocessing.Process):
    ...
    def __init__(self, pd_dict, control):
        ...
        self.server = cb.getCollectDServer(self.q_of_samples)
