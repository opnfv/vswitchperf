...
class Trex(ITrafficGenerator):
    ...
    def run_trials(self, traffic, boundaries, duration, lossrate):
        ...
        while (right - left) > threshold:
            ...
            self._logger.debug("Iteration: %s, packets lost %s, frame rate: %s, throughput_rx_fps: %s," +
                               "frame_loss_percent: %s", iteration, packets_lost,
                               "{:.3f}".format(['frame_rate']), stats['total']['rx_pps'],
