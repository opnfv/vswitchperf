...
class ConnError(Exception):
    """
    Wrapper class for connection error
    """
    # pylint: disable=super-init-not-called
    ...
    def __repr__(self):
        ...
        if self.msg.find(': ') != -1:
            msp, dap = self.msg.split(': ', 1)
            return 'ConnError(message=%s, code=%d, detail=%s)' % (
                msp, self.code, dap)
        return 'ConnError(message=%s, code=%d)' % (self.msg, self.code)
...
class RestHttp(object):
    ...
    def __init__(self, base_url, user=None, password=None, ssl_verify=True,
        ...
        if user and password:
            b64string = (base64.b64encode('%s:%s' % (user,
    ...
    def make_url(self, container=None, resource=None, query_items=None):
        ...
        if isinstance(query_items, (list, tuple, set)):
            ...
            query_items = None
        prr = requests.PreparedRequest()
        prr.prepare_url(url, query_items)
        return prr.url
    ...
    def head_request(self, container, resource=None):
        ...
        try:
            ...
            RestHttp._raise_conn_error(exc)
    ...
    def post_request(self, container, resource=None, params=None, accept=None):
        ...
        try:
            rsp = requests.post(url, data=json.dumps(params), headers=headers,
                                verify=self._verify)
        except requests.exceptions.ConnectionError as exc:
            RestHttp._raise_conn_error(exc)
    ...
    def download_file(self, container, resource, save_path=None, accept=None,
        ...
        file_size_dl = 0
        try:
            with open(save_path, 'wb') as sav_f:
                for buff in rsp.iter_content(chunk_size=16384):
                    sav_f.write(buff)
        except Exception as exc:
            raise RuntimeError('could not download file: ' + str(exc))
        finally:
            rsp.close()
    ...
    # pylint: disable=too-many-branches, maybe-no-member
    ...
    def _uc_to_str(self, data, loc):
        """
        Unicode to string
        """
        if isinstance(data, dict):
            return {self._uc_to_str(k, loc): self._uc_to_str(v, loc)
                    for k, v in data.iteritems()}
        elif isinstance(data, list):
            return [self._uc_to_str(i, loc) for i in data]
        elif isinstance(data, six.string_types):
            if loc:
                return str(data).lower()
            return str(data)
        elif isinstance(data, str) and loc:
            return data.lower()
        return data
    ...
    @staticmethod
    def _raise_conn_error(exc):
        """
        Raise connection error
        """
        if isinstance(exc, requests.exceptions.SSLError):
            raise ConnError(str(exc), -1)
        try:
            msg, err = exc.message
            num, detail = err
            ...
            num = -1
            detail = None
        raise ConnError(msg, num, detail)

    @classmethod
    def __print_req(cls, method, url, headers, params):
        """
        Print The request
        """
