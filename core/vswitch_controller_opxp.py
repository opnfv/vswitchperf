...
"""VSwitch controller for Physical to Tunnel Endpoint to Physical deployment for multi VM scenarios
"""
import logging
import netaddr
...
class VswitchControllerOPXP(IVswitchController):
    ...
    def _setup_decap_encap(self):
        """ Sets up the switch for overlay PXP decapsulation encapsulation test
        """
        self._logger.debug('Setup using %s', str(self._vswitch_class))
        try:
            bridge = S.getValue('TUNNEL_INTEGRATION_BRIDGE')
            ...
            self._vswitch.add_flow(bridge_ext, flow)
            dst_ip_value = netaddr.IPAddress(self._traffic['l4']
                                             ['inner_dstip']).value
            stream = 0
            ...
            for vmindex in range(self._pxp_vm_count):
                ...
                port = vm_ports[vmindex][0]
                flow = add_ports_to_flow(mod_flow_template, phy3_number, port)
                self._vswitch.add_flow(bridge, flow)
                flow = add_ports_to_flow(_FLOW_TEMPLATE, port, phy3_number)
                self._vswitch.add_flow(bridge, flow)
