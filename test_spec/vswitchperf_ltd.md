...
##1. Introduction ##
 ...
 This document is currently in draft form.
  ...
  - RFC 3393 IP Packet Delay Variation for IP Performance Metrics (IPPM)
  - RFC 3918 Methodology for IP Multicast Benchmarking
  - RFC 4737 Packet Reordering Metrics
  - MEF Technical Specification MEF 10.2
...
This section describes the features to be tested ([cf. 2.1](#FeaturesToBeTested)), the test approach ([cf. 2.2](#Approach)); it also identifies the sets of test cases or scenarios ([cf. 2.3](#TestIdentification)) along with the pass/fail criteria ([cf. 2.4](#PassFail)) and the test deliverables ([cf. 2.5](#TestDeliverables)).
  ...
  Characterizing virtual switches (i.e. Device Under Test (DUT) in this document) includes measuring the following performance parameters:
   ...
   - Frame delay introduced by the node and its cumulative effect on E2E networks.
   ...
   - Burst behaviour: measures the ability of the node to buffer packets.
   - Packet re-ordering
   - Availability and capacity of the network element i.e. when the network service is fully “up” and connected:
 ...
 **Note**: For throughput tests unless stated otherwise, test configurations should ensure that traffic traverses the fastpath through the switch, i.e. flows are installed and have an appropriate time out that doesn't expire before packet transmission starts.
 ...
 RFC 3393 is an IETF RFC that outlines IP packet delay variation metric for IP performance metrics
