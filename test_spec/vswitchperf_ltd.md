...
###2.3. Test identification
  ...
  **Description**:
  ...
  This test determines the DUT's maximum forwarding rate without traffic loss for a constant load (fixed length frames at a fixed interval time). The selected frame sizes are those previously defined under [Default Test Parameters](#DefaultParams). The test can also be used to determine the average latency of the traffic.
  ...
  **Metrics Collected**:
  ...
  The following are the metrics collected for this test:
   ...
   - The maximum forwarding rate in Frame Per Second (FPS) and Mbps of the DUT for each frame size with zero packet loss.
...
<br/>
  ...
  **Metrics Collected**:
  ...
  The following are the metrics collected for this test:
   ...
   - The maximum forwarding rate in Frame Per Second (FPS) and Mbps of the DUT for each frame size with zero packet loss and packet modification operations being performed by the DUT.
   - The average latency of the traffic flow when passing through the DUT (if testing for latency).
...
<br/>
  ...
  **Title**: RFC 2544 0% packet loss Throughput Soak Test
  ...
  **Metrics Collected**:
  ...
  The following are the metrics collected for this test:
   ...
   - Throughput distribution of the DUT.
   - Any outliers in the throughput distribution.

<br/>
 - #####Test ID: LTD.Throughput.RFC2544.SoakFrameModification
  **Title**: RFC 2544 0% packet loss Throughput Soak Test with packet modification
...
<br/>
  ...
  **Description**:
  ...
  Although this test can be run as a separate test, the test can also be covered by the latency/packet delay tests, as these tests run for extended periods of time and also provide a measured throughput.
