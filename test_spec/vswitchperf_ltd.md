...
#####2.3.5 Coupling between control path and datapath Tests
  ...
  **Description**:
  ...
  Initially a certain number of flows are installed in the vSwitch. Then over the duration of an RFC2544 throughput test an equal number of flows are added and removed per second. No traffic is passing through these flows, they are simply added and removed.

  The test MUST be repeated with the following initial number of flows installed:
  - < 10
  - 1000
  - 100,000
  - 10,000,000 (or the maximum supported number of flows)

  The test MUST be repeated with the following rates of flow addition and deletion per second:
