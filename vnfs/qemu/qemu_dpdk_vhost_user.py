...
if settings.getValue('QEMU_DIR'):
    QEMU_BIN = os.path.join(settings.getValue('QEMU_DIR'), 'x86_64-softmmu',
                            settings.getValue('QEMU_BIN'))
else:
    QEMU_BIN = settings.getValue('QEMU_BIN')
...
class QemuDpdkVhost(tasks.Process, IVnf):
    ...
    def __init__(self, memory=GUEST_MEMORY, cpus=GUEST_SMP,
        ...
        self._cmd = ['sudo', '-E', self._bin, '-m', str(memory),
                     ...
                     '-device',
                     'virtio-net-pci,mac=00:00:00:00:00:01,netdev=net1',
                     '-netdev',
                     'type=vhost-user,id=net2,chardev=char1,vhostforce',
                     '-device',
                     'virtio-net-pci,mac=00:00:00:00:00:02,netdev=net2',
